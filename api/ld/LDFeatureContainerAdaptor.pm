...
sub fetch_by_Slice {
  ...
  if ($self->db->use_vcf) {
    if (defined $population) {
      if (grep {$_} map {$_->has_Population($population)} @{$self->db->get_VCFCollectionAdaptor->fetch_all}) {
...
#        return $ldFeatureContainer;
      ...
      if ($self->db->use_vcf > 1) { # use vcf only
        my $ldFeatureContainer = $self->_ld_calc(\@genotypes);
        $ldFeatureContainer->name($slice_objects[0]->name());
        return $ldFeatureContainer;
      }
  ...
  if ($in_str eq '') {
    #there is no population, not a human species or not passed as an argument, return the empy container
...
sub _fetch_by_Slice_VCF {
  ...
  foreach my $vc(@{$vca->fetch_all}) {
    ...
    my $vc_genotypes_hash = $vc->_get_all_LD_genotypes_by_Slice_DEV($slice, $population);
    my $vc_genotypes = $vc_genotypes_hash->{genotypes};
    my $vc_ids = $vc_genotypes_hash->{ids};
    while (my ($position, $id) = each %$vc_ids) {
      $ids->{$position} = $id;
    } 

#    my $vc_genotypes = $vc->_get_all_LD_genotypes_by_Slice($slice, $population);

#    print Dumper($vc_genotypes), "\n"; 
#    print scalar keys %$vc_genotypes, "\n";    
...
#    print Dumper(\%pops), "\n";
  ...
  $self->_ld_calc_fast($genotypes, $slice, \%pops, $ids);
...
sub _ld_calc_fast {
  ...
  my $genotypes = shift;
  my $slice = shift;
  my $pops = shift; # sample -> population
  my $ids = shift;
#  print Dumper($vc_genotypes), "\n"; 
#  print scalar keys %$genotypes, "\n";    
#  print Dumper($pops), "\n";
  # create an artificial numerical ID for samples
  my $current_sample_id = 1;
  my %sample_ids;

  my $fh = FileHandle->new('/Users/anjathormann/Documents/DEV/work/api/tmp_ld_in', 'w');

  my $va = $self->db->get_VariationAdaptor();
  my $vfa = $self->db->get_VariationFeatureAdaptor();
  my $pa = $self->db->get_PopulationAdaptor();
  my $population_ids = {};
  foreach my $snp_start (keys %$genotypes) {
    foreach my $sample (keys %{$genotypes->{$snp_start}}) {
      my $gt = $genotypes->{$snp_start}->{$sample};
      ...
      # 0 48903756  48903756  373508  69  AA
        print $fh "0\t$snp_start\t$snp_start\t$pop_id\t$sample_id\t$gt\n";
        $population_ids->{$pop_id} = 1;
      }
    }
  }
  $fh->close();
  my @keys = keys %$population_ids;
  my $pop_id = pop @keys;
  my $population = $pa->fetch_by_dbID($pop_id);
  my $population_name = $population->name();
  my $seq_region_name = $slice->seq_region_name;

  `/Users/anjathormann/Documents/DEV/ensembl-variation/C_code/calc_genotypes </Users/anjathormann/Documents/DEV/work/api/tmp_ld_in >/Users/anjathormann/Documents/DEV/work/api/tmp_ld_out`;
  # ($population_id,$ld_region_id,$ld_region_start,$ld_region_end,$r2,$d_prime,$sample_count) = split /\s/;

  $fh = FileHandle->new('/Users/anjathormann/Documents/DEV/work/api/tmp_ld_out', 'r');
  my $variation_names = {};
  ...
  while (<$fh>) {
    ...
    if ($r2 > 0.8) {
      my $id1 = $ids->{$ld_region_start};
      my $vf1 = $self->get_variation_feature_object($ld_region_start, $id1, $va, $vfa); 
      my $id2 = $ids->{$ld_region_end};
      my $vf2 = $self->get_variation_feature_object($ld_region_end, $id2, $va, $vfa);
      $variation_names->{$id1} = 1;
      $variation_names->{$id2} = 2;
      push @ld_feature_container, {
        'seq_region_name' => $seq_region_name,
        'population_name' => $population_name,
        'variation1' => $id1,
        'location1' => $ld_region_start,
        'variation2' => $id2,
        'location2' => $ld_region_end,
        'r2' => $r2,
        'd_prime' => $d_prime,
      };
    }
  ...
  print Dumper(\@ld_feature_container), "\n";
  print 'count ld_feature hashes: ', scalar @ld_feature_container, "\n";

  print 'variant count ', scalar keys %$variation_names, "\n"; 
  foreach my $name (keys %$variation_names) {
    if ($name =~ /;/) {
      print $name, "\n";
#      my $variation_object = $va->fetch_by_name($name);
    }
  }

}

sub get_variation_feature_object {
  my $self = shift;
  my $position = shift;
  my $ids = shift;
  my $va = shift;
    ...
    foreach my $population (keys %{$alleles_variation->{$snp_start}}){
      ...
      if (keys %{$alleles_variation->{$snp_start}{$population}} == 2){
        ...
        foreach my $sample_id (keys %{$sample_information->{$population}{$snp_start}}){
          print join("\t",$previous_seq_region_id, $snp_start, $snp_start, $population, $sample_id, $sample_information->{$population}{$snp_start}{$sample_id}{genotype})."\n";
        ...
        print "dont convert\n";
        print Dumper(\%{$alleles_variation->{$snp_start}{$population}}), "\n";
      }
    }
  }
