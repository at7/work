...
sub print_variation_gt_counts {
  ...
  foreach my $population_name (keys %$populations) {
    ...
    foreach my $sample (@samples) {
      $sample_2_population->{$sample->dbID}->{$population_name} = 1;
  ...
  my $cache = update_genotype_cache("/hps/nobackup/production/ensembl/anja/release_89/hapmap/38/genotypes/17.txt");

  my $fh = FileHandle->new('/hps/nobackup/production/ensembl/anja/release_89/hapmap/38/variation_sample_count_chrom17.txt', 'w');

  foreach my $variation_id (keys %$cache) {
    my $sample_in_populations_counts = {};
    foreach my $sample_id (keys %{$cache->{$variation_id}}) {
      foreach my $population_name (keys %{$sample_2_population->{$sample_id}}) {
        $sample_in_populations_counts->{$population_name}++;
      }
    }     

    foreach my $population_name (keys %$sample_in_populations_counts) {
      my $count = $sample_in_populations_counts->{$population_name};
      print $fh join("\t", $variation_id, $population_name, $count), "\n";
    }
  }   
  $fh->close();
}
