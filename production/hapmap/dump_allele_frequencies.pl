...
sub dump_allele_frequencies {
  ...
  my $sth = $dbc->prepare(qq{
    SELECT a.variation_id, ac.allele, a.frequency, a.count, a.population_id
  ...
  $sth->execute($handle);
  my ($variation_id, $allele, $frequency, $count, $population_id);
  $sth->bind_columns(\($variation_id, $allele, $frequency, $count, $population_id));
  while ($sth->fetch) {
    if ($population_id) {
      if ($fhs->{$population_id}) {
        my $fh = $fhs->{$population_id};
        print $fh join("\t", ($variation_id, $allele, $frequency, $count)), "\n";
      }
    }
  }
